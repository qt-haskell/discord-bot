...
from asyncpg import Pool, Record, InterfaceError
...
class DefaultConnectionStrategy(ConnectionStrategy):
    ...
    if TYPE_CHECKING:
        # Bit of lying but correct enough
        _connection: PoolConnectionProxy[Record]
    ...
    async def release_connection(self) -> None:
        if self._transaction is not None:
            try:
                ...
                log.exception("Failed to commit transaction due to exception", exc_info=True)
...
class PostgreSQLManager(BaseManager):
    ...
    def __init__(
        ...
        super().__init__(DefaultConnectionStrategy(pool, timeout=timeout))
